    ...
    Converter<Map<String, String>, OAuth2AccessTokenResponse>
{
    ...
    public OAuth2AccessTokenResponse convert(Map<String, String> tokenResponseParameters)
    {
        ...
        if (OAuth2AccessToken.TokenType.BEARER.getValue().equalsIgnoreCase(tokenResponseParameters.get(
            OAuth2ParameterNames.TOKEN_TYPE)))
        {
        ...
        return OAuth2AccessTokenResponse.withToken(accessToken).tokenType(accessTokenType).expiresIn(expiresIn).scopes(
            scopes).additionalParameters(additionalParameters).build();
    }
